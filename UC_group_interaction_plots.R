...
# 2-way split
combined_matrix[Meara>=median(Meara), Meara2 := 'High']
combined_matrix[Meara<median(Meara), Meara2 := 'Low']
combined_matrix[, Meara2 := factor(Meara2, levels=c('Low', 'High'))]
combined_matrix[BarChartLit>=median(BarChartLit), VisLit2 := 'High']
combined_matrix[BarChartLit<median(BarChartLit), VisLit2 := 'Low']
...
ggplot(sss, aes(x=Meara2, y=numtransfrom_Refs, group=group)) + 
  ...
  geom_errorbar(aes(ymin=numtransfrom_Refs-ci*0.8, ymax=numtransfrom_Refs+ci*0.8, color=group), width=.1) +
...
ggplot(sss, aes(x=VisLit2, y=numtransfrom_labels, group=group)) + 
  geom_line(aes(color=group)) +
  geom_point(aes(color=group)) + 
  geom_errorbar(aes(ymin=numtransfrom_labels-ci*0.8, ymax=numtransfrom_labels+ci*0.8, color=group), width=.1, position = position_dodge(width = 0.05)) +
...
ggplot(sss, aes(x=VisLit2, y=numtransfrom_Non_relevant_bars, group=group)) + 
  geom_line(aes(color=group)) +
  geom_point(aes(color=group)) + 
  geom_errorbar(aes(ymin=numtransfrom_Non_relevant_bars-ci*0.8, ymax=numtransfrom_Non_relevant_bars+ci*0.8, color=group), width=.1, position = position_dodge(width = 0.05)) +
...
ggplot(sss, aes(x=VisLit2, y=numtransfrom_Refs, group=group)) + 
  geom_line(aes(color=group)) +
  geom_point(aes(color=group)) + 
  geom_errorbar(aes(ymin=numtransfrom_Refs-ci*0.8, ymax=numtransfrom_Refs+ci*0.8, color=group), width=.1, position = position_dodge(width = 0.05)) +
...
combined_matrix <- combined_matrix[order(BarChartLit)]
bcl_raw <- bcl_raw[order(BarChartLit_raw)]
pivot_lowA <- combined_matrix[nrow(combined_matrix)*(1/3)]$BarChartLit
pivot_lowB <- bcl_raw[nrow(bcl_raw)*(1/3)]$BarChartLit_raw
pivot_highA <- combined_matrix[nrow(combined_matrix)*(2/3)]$BarChartLit
pivot_highB <- bcl_raw[nrow(bcl_raw)*(2/3)]$BarChartLit_raw

combined_matrix[BarChartLit<pivot_lowA, VisLit3 := 'Low']
combined_matrix[BarChartLit==pivot_lowA & BarChartLit<pivot_lowB, VisLit3 := 'Low']
combined_matrix[BarChartLit<pivot_highA & BarChartLit>pivot_lowA, VisLit3 := 'Med']
combined_matrix[BarChartLit==pivot_lowA & BarChartLit>=pivot_lowB, VisLit3 := 'Med']
combined_matrix[BarChartLit>=pivot_highA, VisLit3 := 'High']
combined_matrix[, VisLit3 := factor(VisLit3, levels=c('Low', 'Med', 'High'))]


combined_matrix <- combined_matrix[order(Meara)]
pivot_low <- combined_matrix[nrow(combined_matrix)*(1/4)]$Meara
pivot_high <- combined_matrix[nrow(combined_matrix)*(3/4)]$Meara
...
means1 <- lsmeans(test, pairwise ~ VisLit3|group, adjust = 'none')
means2 <- lsmeans(test, pairwise ~ group|VisLit3, adjust = 'none')
means2 <- lsmeans(test, pairwise ~ VisLit3|group + group|VisLit3, adjust = 'fdr')
means1$contrasts
means2$contrasts
