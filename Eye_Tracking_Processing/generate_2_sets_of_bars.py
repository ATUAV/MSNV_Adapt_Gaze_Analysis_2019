...
1 mark or the legend, with the aoi name set to its id. Each has 5 points, 
although the 5th point is the same the 1st point and is not actually needed. The
last line simply gives the concatenation of all the above AOIs' coordinates, and 
does not actually define an AOI we want.
'''
import pandas as pd
import json
...
msnv_ids = [27, 60, 11, 30, 62, 72, 28, 74, 5, 20, 76, 66, 9, 3, 18]
#msnv_ids = [30]
offsets = (604,80)
...
for msnv, interventions in intervention_grouped:
    ...
    for aoi in aois:
        ...
        for vertex in aoi_coord:
            string = '\t' + str(vertex[0]) + ',' + str(vertex[1])
            fout.write(string)
            if (aoi_name != 'legend'):
                if aoi_name in relevant_indices:
                    rel_overall_list += string
                else: non_rel_overall_list += string
        fout.write('\n')
