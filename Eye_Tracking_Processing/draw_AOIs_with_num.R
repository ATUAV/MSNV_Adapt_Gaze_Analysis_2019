...
for (msnv in mmd_list) {
  ...
  bars <- my_aois[nrow(my_aois)]  
  bars <- str_split(as.character(bars), "\t")[[1]][-1]
  bars <- bars[!sapply(bars, function(x) x == "")]
  
  size <- length(bars) / 5
  
  for (i in 0:(size - 1)) {
    top <- as.integer(str_split(bars[5 * i + 1], ",")[[1]][[2]])
    left <- as.integer(str_split(bars[5 * i + 1], ",")[[1]][[1]])
    width <- as.integer(str_split(bars[5 * i + 2], ",")[[1]][[1]]) - as.integer(str_split(bars[5 * i + 1], ",")[[1]][[1]])
    height <- as.integer(str_split(bars[5 * i + 3], ",")[[1]][[2]]) - as.integer(str_split(bars[5 * i + 2], ",")[[1]][[2]])
